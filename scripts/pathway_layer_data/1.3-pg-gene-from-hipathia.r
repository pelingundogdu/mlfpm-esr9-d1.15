...
#             df_merge = merge(df_temp,df_merge, by='entrez', all=T)
            if (nrow(df_merge)==0){
                ...
                if (length(l_sub) != 0){
                    df_merge = merge(df_temp,df_merge, by='entrez', all.y= TRUE, all.x=TRUE)
                }
            }
...
#         rownames(df_path_genes) <- 1:nrow(df_path_genes)
    ...
    #     Exporting gene set of each pathways (93 txt file for using pathways for hsa)
        write.table(df_merge, paste0(output_folder_detail,list_path_id[all_pathways_],'_gene_list.txt'), sep=',', row.names=FALSE)
    }
