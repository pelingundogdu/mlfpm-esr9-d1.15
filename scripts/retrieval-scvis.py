...
def compute_retrieval_scores(model_path, n_epochs, analysis, snorm, ref_gene_file, sub_output_dir, out_file, gnorm, scaler, data_file_path, data_training, alternative_model_csv_path):
    ...
    code, time = encode_data(model_path, n_epochs, analysis, all_data, scaler, data_training, alternative_model_csv_path)
    print(all_data)
    print("all_data.shape: ", all_data.shape)
...
#     out_file = os.path.splitext(out_file)[0]
    ...
    for ds in dataset_sets:
        print(f'{ds}/{len(list(set(all_weights)))}')
...
def encode_alternative_algorithm(path):
    code = pd.read_csv(path, sep='\t', index_col=0)
    return np.array(code)

def encode_data(model_path, n_epochs, analysis, data, scaler, data_training, alternative_model_csv_path):
    ...
    elif re.search('scvis', analysis):
        print('\nRETRIEVAL ANALYSIS IS PERFORMING for alternative model scvis')
        code = encode_alternative_algorithm(alternative_model_csv_path)
    else:
...
def main(model_path, n_epochs, analysis, snorm, ref_gene_file, gnorm, scaler, design_name, data_training, alternative_model_csv_path):
...
#         \ndata_training, {data_training.shape}
        ...
        sub_output_dir=f'./reports/retrieval/scvis_exper_'+experiment_name
    ...
    #     print(f'bool(snorm) --> {bool(eval(snorm))}')
        ...
        time_encoding = compute_retrieval_scores(model_path, n_epochs, analysis, snorm, ref_gene_file, sub_output_dir, design_name, gnorm, scaler, data_file_path, data_training, alternative_model_csv_path)
...
if __name__ == "__main__":
    _, model_path, n_epochs, analysis, snorm, ref_gene_file, gnorm, scaler, design_name, data_training, alternative_model_csv_path = sys.argv

    main(model_path, n_epochs, analysis, bool(eval(snorm)), ref_gene_file, bool(eval(gnorm)), bool(eval(scaler)), design_name, data_training, alternative_model_csv_path)
