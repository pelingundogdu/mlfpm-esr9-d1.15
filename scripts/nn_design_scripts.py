    ...
    try:
        ...
        with strategy.scope():
            ...
            if (second_layer==True):
                ...
                model.add(keras.layers.Dense(200, activation=select_activation, name='layer2'))
...
#                 optimizer = keras.optimizers.Adam(lr=1e-7, beta_1=0.9, beta_2=0.999)
...
class tuning(kt.HyperModel):
    ...
    def build(self, hp):
        ...
        hp_momentum = hp.Choice('momentum', values=[0.6, 0.8, 0.9, 1.0] )
...
#         hp_decay = hp.Choice('decay', values=[1e-6] )
        ...
        optimizer = keras.optimizers.SGD(lr = hp_learning_rate
